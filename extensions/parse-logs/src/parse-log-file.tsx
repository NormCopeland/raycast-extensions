  ...
  return (
    ...
    >
      <Form.TextField
        id="searchQuery"
        title="Search Query"
        placeholder="Enter search term(s) - use commas to separate multiple values (e.g., FATAL,ERROR)"
...
async function parseLogFiles(
  ...
  // Parse search query to handle multiple comma-separated terms
  ...
  // Filter lines containing any of the search terms
  const matchingLines = lines.filter((line) => 
    searchTerms.some((term) => line.toLowerCase().includes(term))
  );

  if (matchingLines.length === 0) {
    const termDescription = searchTerms.length === 1 ? "term" : "any of the terms";
    throw new Error(`No lines found containing ${termDescription} "${searchQuery}"`);
  }
    ...
    <Detail
      ...
      metadata={
        <Detail.Metadata>
          ...
          <Detail.Metadata.Label title="Search Terms" text={`${result.searchTerms.length === 1 ? "Single term" : `${result.searchTerms.length} terms`} - OR logic`} />
